...
if files:
    for file in files:
        ...
        if st.checkbox(f"Remove Duplicates - {file.name}", key=f"remove_duplicates_{file.name}"):
            df = df.drop_duplicates()
            st.success("Duplicates Removed")
            st.dataframe(df.head())
        ...
        select_columns = st.multiselect(
            ...
            df.columns, 
            default=df.columns,
            key=f"select_columns_{file.name}"  # Unique key add karein
        )
        df = df[select_columns]
        st.dataframe(df.head())

        if st.checkbox(f"Show Charts - {file.name}", key=f"show_charts_{file.name}") and not df.select_dtypes(include="number").empty:
            st.bar_chart(df.select_dtypes(include="number").iloc[:, :2])

        format_choice = st.radio(
            f"Convert {file.name} to:", 
            ["csv", "Excel"], 
            key=f"format_choice_{file.name}"  # Unique key add karein
        )
